...
local function remove_path(Location, SkipCheck)
    if os.execute(Configuration.Settings.SuperuserCommand .. "test -e " .. Location) or SkipCheck then
        ...
        if Configuration.Settings.RemovePathConfirmation == true then
            io.write(Colours.Yellow ..
            "[INPUT REQUIRED] Are you sure you would like to REMOVE the path: " .. Location .. " (y/n) " .. Colours
            .Reset);
...
local function create_path(Location)
    ...
    if Configuration.Settings.AddPathConfirmation == true then
        io.write(Colours.Yellow ..
        "[INPUT REQUIRED] Are you sure you would like to CREATE the path: " .. Location .. " (y/n)" .. Colours.Reset);
        Confirmation = ensure_confirmation();
    end
    if Confirmation then
...
if CacheDirectoryExists == nil then
    print("[LOG] (PLEASE CHECK THE DIRECTORY IS NOT DEFAULT) Cache Directory doesn't exist, generating a new one at: " ..
    Configuration.Settings.CachePath .. CacheFileName);
...
if CacheFileExists == nil then
    print("[LOG] Cache File doesn't exist, generating a new one at: " .. Configuration.Settings.CachePath ..
    CacheFileName);
...
for SymlinkPath, SourcePath in pairs(Configuration.Symlinks) do
    ...
    if os.execute(Configuration.Settings.SuperuserCommand .. "test -e " .. SymlinkPath) == nil then
        ...
        if Configuration.Settings.AddSymlinkConfirmation then
            io.write(Colours.Yellow ..
            "[INPUT REQUIRED] Are you sure you would like a symlink from Source: " ..
            SourcePath .. " Symlink File: " .. SymlinkPath .. " (y/n)" .. Colours.Reset);
            Confirmation = ensure_confirmation();
        end
        if Confirmation then
            ...
            if os.execute(Configuration.Settings.SuperuserCommand .. "ln -s " .. SourcePath .. " " .. SymlinkPath) then
                print(Colours.Green ..
                "[LOG] Created Symlink, Source: " .. SourcePath .. ", Symlink File: " .. SymlinkPath .. Colours.Reset);
            else
                print(Colours.Red ..
                "[ERROR] Failed to Create Symlink, Source: " ..
                SourcePath .. ", Symlink File: " .. SymlinkPath .. Colours.Reset)
            end
        end
    end
